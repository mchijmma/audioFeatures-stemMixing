...
#gStemFeatures = getFeatures(gStemStereoAudio, type='stereo')

#gRawFeatures = getFeatures(gRawStereoAudio, type='stereo')

# Calculates statistical values of the features.
#def getStatisticalFeatures(_dict_features):
#  _features = {}
#  
#  exception = {'lowLevel.melbands' : ['min', 'max', 'median', 'mean',
#                                            'var', 'skew', 'dmean',
#                                            'dvar', 'dmean2', 'dvar2'],
#                                            'lowLevel.gfccbands' :['min', 'max', 'median', 'mean',
#                                            'var', 'skew', 'dmean',
#                                            'dvar', 'dmean2', 'dvar2'],
...
#              #_features[name].set(_name_feature+'.stdev', np.asscalar(_features[name][_name_feature+'.stdev']))
#              #_features[name].add(feature+'.stdev', np.sqrt(_feature[name][feature+'.var']))                                       
#
#    
#  
#  return _features

#gStemFeaturesStats = getStatisticalFeatures(gStemFeatures)

#gRawFeaturesStats = getStatisticalFeatures(gRawFeatures)


# Saves trimmed tracks in respective folders, creates YAML with raw track selected,
# and time in seconds of the trimming. 
#def saveFeatureStats():
#
## Create ddcit of paths for raw tracks and stems
#    raw_paths = dict()
#    stem_paths = dict()
#    a = dict()
#    b = dict()
#    pool = essentia.Pool()
#    
#    pool_features = essentia.Pool()
#
#    path = './Music/Data/MedleyDB/Features/%s/%s/' % (kInstrument, kType)
#    
#    if not os.path.exists(path):
#        os.makedirs(path) 
#    
#    for name, _path in gRawPath.items():
#        pool.set(name+'.'+kInstrument+'.raw_path', _path)
#        
#    for name, _path in gStemPath.items():
#        pool.set(name+'.'+kInstrument+'.stem_path', _path)
#    
#    
#    for name, _pool in gRawFeaturesStats.items():
#        for feature_name in _pool.descriptorNames():
#          
#            pool_features.add(name+'.'+kInstrument+'.'+'raw'+'.'+feature_name, _pool[feature_name])
#            
#    
#    for name, _pool in gStemFeaturesStats.items():
#        for feature_name in _pool.descriptorNames():
#          
#            pool_features.add(name+'.'+kInstrument+'.'+'stem'+'.'+feature_name, _pool[feature_name])
#        
#         
#        
#    
#    path_info = path + 'info.tracks.' + str(datetime.now()).split('.')[0]
#    
#    path_features = path + 'features.stats'# + '.'+str(datetime.now()).split('.')[0]
#    
#       
#    YamlOutput(filename = path_info, writeVersion = False, format = 'json')(pool)    
#        
#    YamlOutput(filename = path_features, writeVersion = False, format = 'json')(pool_features)
...
# returns pool with all the relevant features. 
#def getInfoTracks():
#  
#  pool = essentia.Pool()  
#  i = 0
#  for name in gInputPool.descriptorNames():
#    
#    #If a feature is an array and stats were not calculated, it's dropped. if is the spectrum is drop aswell
#    #str and list type of features are not taking into account
#    
#      if (isinstance(gInputPool[name][0], np.ndarray) and len(name.split('.')) >= 6):
#          if 'spectrum' not in name:
#              
#              _pool = sortArray(gInputPool[name][0], name)
#                          
#              pool.merge(_pool)
#              #pool.add(name, gInputPool[name][0])
#          
#      elif isinstance(gInputPool[name][0], np.float32):
#          pool.add(name, gInputPool[name][0])
#          
#      i += 1
#      
#      if i % 688 == 0:
#          
#          print '%s # %d track info added' % (kInstrument, i/688)
#          
#      
#       
#      
...
# How to do normalization, should I normalize each array ? maybe. should i take statistics of every array or just every component?    

#def getDictFeatures():
#  
#  dict_features = OrderedDict()
#  dict_labels = OrderedDict()
#  f = len(gTotalListFeatures)
#  i = 0  
#  for name in gTotalListFeatures:
#    
#      
#      dict_name = '.'.join(name.split('.')[0:3])
#      dict_features.setdefault(dict_name, []).append(gPool[name])
#      i += 1
#      
#      if i % kFeatures == 0:
#        
#          print '%s # %d track added to dict' % (kInstrument, i/kFeatures)
#        
#        
#    
...
#          _last = name.split('.')[-1]
#          if name2 in name and ( _last == '1' or _last == '0' or _last == 'median'):
#              dict_name = '.'.join(name.split('.')[0:3])
#              
#              dict_features.setdefault(dict_name, []).append(gPool[name])
...
#  
#    
#  return dict_features, name_features 
  
#def getArrayFeatures(dict_features):
#  
#  dictx = OrderedDict()
#
#  x = []
...
#      
#      if 'raw' in name:
#          #dictx.setdefault(name, []).append((dict_features[name],0))
#          x.append(dict_features[name])
#          y.append(0)
#      else:
#          #dictx.setdefault(name, []).append((dict_features[name],1))
#          x.append(dict_features[name])
#          y.append(1)
#      
#      
#  
#  
#  #for n in dictx.values():
#  
#      #x.append(n[0][0])
#      #y.append(n[0][1])
#  
#  x = np.asarray(x).reshape(len(gListTracks), -1)
#  
...
#
#  if i == n-1:
#    y.append( (l[i]+l[i-1])*(s[i]-s[i-1]) / (l[i]+l[i-1]) )
#  else:
#    y.append( (l[i]+l[i+1])*(s[i+1]-s[i]) / (l[i]+l[i+1]) )
#
#y = np.asarray(y)
#
#
##%%
#mean = np.sum(l*f)/np.sum(w)
#var = np.sum(w*np.power(f-mean,2))/np.sum(w)
##%%
#l2  = np.append(l[1::],l[-2])
#s2 = np.append(s[1::],s[-2])
##%%
#der = (l+l2)*(s2-s)/(l+l2)
...
#The spectral_centroid, spectral_kurtosis, spectral_spread, spectral_skewness,
#dissonance, spectral_entropy, spectral_contrast_coeffs, and
#spectral_contrast_valleys are computed with an equal-loudness filter applied to the signal.
...
##Plot waveforms.
#_idx = 1
#_name = gNameTracksOriginal[_idx]
#_path_raw = gRawPath[_name]
#_path_stem = gStemStereoPath[_name]
##gNameFeatures = gStemFeatures[_name].descriptorNames()  
###%%
#plotStemRawTrack(gNameTracksOriginal[_idx])
#%%
##Plays files.
#_args = ['play', _path_raw]
#sox.core.play(_args)
##%%
#_args = ['play', _path_stem]
#sox.core.play(_args)  
##%%
#_feature = 'lowLevel.gfcc'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T[1:,:], gRawFeatures[_name][_feature].T[1:,:], type = 'coeffs', power = False, yaxis = 'linear')
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.gfccbands'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T,  type = 'spectrum', power = False, yaxis = 'linear')
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.mfcc'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T[1:,:], gRawFeatures[_name][_feature].T[1:,:], type = 'coeffs')
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.spectrum'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'spectrum')
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.melbands'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'coeffs', power = False)
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.erbbands'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'spectrum')
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.relative_specific_loudness'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'coeffs', power = False)
#plt.title(_feature, loc='right')
#
##%%
#_feature = 'tonal.hpcp'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, hop_size = 2048, type = 'coeffs')
#plt.title(_feature, loc='right')

#%%
#_feature = 'lowLevel.sccoeffs'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'coeffs', power = False)
#plt.title(_feature, loc='right')
##%%
#_feature = 'lowLevel.scvalleys'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'coeffs', power = False)
#plt.title(_feature, loc='right')
#
##%%
#_feature = 'lowLevel.spectral_centroid'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_crest'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_entropy'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_skewness'
#plotFeature(_feature, _name)
#%%
#_feature = 'dynamic.dynamic_complexity'
#plotFeature(_feature, _name)
#print np.mean(gStemFeatures[_name][_feature])
#print gStemFeatures[_name][_feature+'2']
#print np.mean(gRawFeatures[_name][_feature])
#print gRawFeatures[_name][_feature+'2']
#%%
#_feature = 'lowLevel.spectral_kurtosis'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.dissonance'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_strongpeak'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.zerocrossingrate'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_rms'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_complexity'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_rolloff'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_decrease'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_energy'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_energyband_low'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_energyband_middle_low'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_energyband_middle_high'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_energyband_high'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_flatness_db'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.spectral_flux'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.barkbands_crest'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.barkbands_flatness_db'
#plotFeature(_feature, _name)
##%%
#_feature = 'sfx.inharmonicity'
#plotFeature(_feature, _name)
##%%
#%%
#_feature = 'lowLevel.specific_sharpness'
#plotFeature(_feature, _name)
#%%
#_feature = 'sfx.tristimulus'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'coeffs', power = False)
#
##%%
#_feature = 'sfx.oddtoevenharmonicenergyratio'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.barkbands_kurtosis'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.barkbands_skewness'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.barkbands_spread'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.melbands_kurtosis'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.melbands_skewness'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.melbands_spread'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.melbands_flatness_db'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.melbands_crest'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.erbbands_kurtosis'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.erbbands_skewness'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.erbbands_spread'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.erbbands_flatness_db'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.erbbands_crest'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.hfc'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.pitch'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.pitch_instantaneous_confidence'
#plotFeature(_feature, _name)
##%%
#_feature = 'lowLevel.pitch_salience'
#plotFeature(_feature, _name)
##%%
#_feature = 'tonal.thpcp'
#plt.hist(gStemFeatures[_name][_feature], facecolor='black', alpha=0.5)
#plt.hist(gRawFeatures[_name][_feature], facecolor='cyan', alpha=0.5)
#plt.title(_feature, loc='right')
#plt.show()
##%%
#_feature = 'tonal.hpcp_crest'
#plotFeature(_feature, _name, hop_size = 2048)
##%%
#_feature = 'tonal.hpcp_entropy'
#plotFeature(_feature, _name, hop_size = 2048)

#%%
#_feature = 'tonal.chords_strength'
#plotFeature(_feature, _name, hop_size = 2048)
##%%
#_feature = 'tonal.chords_histogram'
#plt.hist(gStemFeatures[_name][_feature], facecolor='black', alpha=0.5)
#plt.hist(gRawFeatures[_name][_feature], facecolor='cyan', alpha=0.5)
#plt.title(_feature, loc='right')
#plt.show()
##%%
#_feature = 'rhythm.beats_loudness_band_ratio'
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T, gStemFeatures[_name][_feature].T, type = 'coeffs', power = False)
#plt.title(_feature, loc='right')
##%%
#_feature = 'rhythm.beats_loudness'
#plt.hist(gStemFeatures[_name][_feature], facecolor='black', alpha=0.5)
#plt.hist(gRawFeatures[_name][_feature], facecolor='cyan', alpha=0.5)
#plt.title(_feature, loc='right')
#plt.show()
##%%
#_feature = 'panning.coeffs'
#plt.hist(gStemFeatures[_name][_feature], facecolor='black', alpha=0.5)
#plt.hist(gRawFeatures[_name][_feature], facecolor='cyan', alpha=0.5)
#plt.title(_feature, loc='right')
#plt.show()
##%%
#_feature = 'rhythm.beats_loudness'
#plot(gStemFeatures[_name][_feature], 'k', gRawFeatures[_name][_feature], 'c' )
#plt.title(_feature, loc='right')
#
#
##%%
#_feature = 'dynamic.larm'
#plotFeature(_feature, _name, hop_size = 2048)
##%%
#_feature = 'dynamic.leq'
#plotFeature(_feature, _name, hop_size = 2048)
##%%
#_feature = 'dynamic.loudness'
#plotFeature(_feature, _name, hop_size = 2048)
##%%
#_feature = 'dynamic.loudness_vickers'
#plotFeature(_feature, _name, hop_size = 2048)
##%%
#_feature = 'dynamic.rms'
#plotFeature(_feature, _name, hop_size = 2048)
#
## PLOT BAR FEATURES
#
##%%
#_feature = 'dynamic.dynamic_complexity'
#plotBarFeatures(_feature)
##%%
#_feature = 'dynamic.dynamic_complexity_loudness'
#plotBarFeatures(_feature)
##%%
#_feature = 'dynamic.replay_gain'
#plotBarFeatures(_feature)
##%%
#_feature = 'lowLevel.average_loudness'
#plotBarFeatures(_feature)
##%%
#_feature = 'sfx.pitch_after_max_to_before_max_energy_ratio'
#plotBarFeatures(_feature)
##%%
#_feature = 'sfx.pitch_centroid'
#plotBarFeatures(_feature)
##%%
#_feature = 'sfx.pitch_max_to_total'
#plotBarFeatures(_feature)
##%%
#_feature = 'sfx.pitch_min_to_total'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.chords_changes_rate'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.chords_number_rate'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.key_strength'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.tuning_diatonic_strength'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.tuning_equal_tempered_deviation'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.tuning_nontempered_energy_ratio'
#plotBarFeatures(_feature)
##%%
#_feature = 'tonal.tuning_frequency'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.confidence'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.first_peak_bpm'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.first_peak_spread'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.first_peak_weight'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.second_peak_bpm'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.second_peak_spread'
#plotBarFeatures(_feature)
##%%
#_feature = 'rhythm.second_peak_weight'
#plotBarFeatures(_feature)
#
#
#
...
##check specific.
#_idx = 0
#_name = gNameTracks[_idx]
#_path_raw = gRawPath[_name]
#_path_stem = gStemStereoPath[_name]
#gNameFeatures = gStemFeatures[_name].descriptorNames()  
##%%
#_feature = 'panning.CD'
##White = left panned, Gray = center, Black = right panned.
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T,
#                    gStemFeatures[_name][_feature].T,
#                    type = 'coefss', power = False, yaxis = 'linear', cmap = 'Greys', colorbar = True)
#plt.title(_feature, loc='right')
##%%
#_feature = 'panning.SPS'
##White = left panned, Gray = center, Black = right panned.
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T,
#                    gStemFeatures[_name][_feature].T,
#                    type = 'coeffs', power = False, yaxis = 'log', cmap = 'Greys', colorbar = True)
#plt.title(_feature, loc='right')
##%%
#_feature = 'panning.SSPS'
##White = left panned, Gray = center, Black = right panned.
#plotStemRawSpectrum(gRawFeatures[_name][_feature].T,
#                    gStemFeatures[_name][_feature].T,
#                    type = 'spectrum', power = False, yaxis = 'linear')# cmap = 'Greys', colorbar = True)
#plt.title(_feature, loc='right')
##%%
#_feature = 'panning.P_total'
#plotFeature(_feature, _name)
##%%
#_feature = 'panning.P_low'
#plotFeature(_feature, _name)
##%%
#_feature = 'panning.P_medium'
#plotFeature(_feature, _name)
##%%
#_feature = 'panning.P_high'
#plotFeature(_feature, _name)
